...
if suffix == '_':
    prefix_name = 'mice_j03j10_%s_%s_ep%s_%s_bs%s_lr%s_wd%s_validfld%s' \
                  % (neuralnet_used, dataset_name, start_epoch, last_epoch, batch_size, learning_rate, w_decay, valid_fold_train)
else:
    prefix_name = 'mice_j03j10_%s_%s_ep%s_%s_bs%s_lr%s_wd%s_validfld%s_%s' \
                  % (neuralnet_used, dataset_name, start_epoch, last_epoch, batch_size, learning_rate, w_decay, valid_fold_train, suffix)
...
def get_predictions_mat(neuralnet, loss_criterion, data_loader, prefixname, dataset):
    ...
    fig.text(0.12, 0.965, '%s, %s, best model btwn ep %s and %s,\n%s, lr=%s, bs=%s, w_decay=%s, validfld %s'
             % (echeance, neuralnet_used, start_epoch, last_epoch, loss_criterion, learning_rate, batch_size, w_decay,
                valid_fold_train), ha='left', va='top', size='x-small')
...
if neuralnet_used == 'SqueezeNet':
    ...
    neural_net.classifier[1] = torch.nn.Conv2d(512, 22, kernel_size=(1, 1), stride=(1, 1))
    ...
    neural_net.classifier[6] = torch.nn.Linear(in_features=4096, out_features=22, bias=True)
