...
# Selection de variables avec Boruta pour la classification binaire naloxone/petite lipectomie a partir des features
# des images d'un ensemble de donnees qu'on charge depuis un fichier pickle (.pkl).
# Genere : - 1 fichier texte de resume de l'entrainement
#          - 3 fichiers npy contenant les valeurs des features selectionnees par Boruta, les valeurs de ytrain et
# le masque des features conserves.

parser = argparse.ArgumentParser()
parser.add_argument('-jobid', '--jobid', default='230323_J10NJ10PL_set4_trainmosTe', type=str)
...
torch.manual_seed(0)
# dossier d'images utilise (en fait c'est le dossier contenant les fichiers pkl) :
dataset_name = '230323_J10NJ10PL_set4_trainmosTe'

if suffix == '_':
    prefix_name = 'binary_classif_ML_%s_%s' % (dataset_name, jobid_toread)
else:
    prefix_name = 'binary_classif_ML_%s_%s_%s' % (dataset_name, jobid_toread, suffix)

"""
# A commenter pour la creation de Xtestfiltered
...
print('binclassif_varselection.py')
f.write('binclassif_varselection.py\n')
...
# -- Creation des listes X_train (features des images) et y_train (labels nalox ou ptlip des images) --
"""
# Pickle file loading
...
def extract_features(jobidtoread, datasetname):
    Xtrain = []
    ytrain = []
    ...
    for echeance in ['j03', 'j10']:
        for classe in ['nalox', 'ptlip']:
            ...
            # img_tiles = os.listdir(os.path.join('./output_features_%s' % datasetname, echeance, classe))
            for img_tile_name in img_tiles:
                ...
                for ch in ['ch1', 'ch2']:
                    ...
                    # list_pkl_files = os.listdir(os.path.join('./output_features_%s' % datasetname, echeance, classe,
                    #                                          img_tile_name, ch))
                    for pklfile in list_pkl_files:
                        ...
                        # pklfile_path = os.path.join('./output_features_%s' % datasetname, echeance, classe, img_tile_name,
                        #                            ch, pklfile)
...
X_train, y_train = extract_features(jobid_toread, dataset_name)
imgnb = len(X_train)
print('len X_train:', imgnb)
f.write('len X_train: {val} \n'.format(val=imgnb))
print('len y_train :', len(y_train))
f.write('len y_train: {val1} \n'.format(val1=len(y_train)))
...
# on recupere les features selectionnes sur l'ensemble entier et on va les appliquer aux Xtrain, ytrain correspondant
# au sous-ensemble qui nous interesse
# A faire pour train et test de notre sous-ensemble.

npy_path_Xtestfiltered = './npy_files/Xtestfiltered_%s.npy' % prefix_name
npy_path_ytest = './npy_files/ytest_%s.npy' % prefix_name
npy_path_featmask = './npy_files/featmask_binary_classif_ML_230323_trainmosTV_0.npy'
# on utilise ce fichier prealablement cree pour la selection de variable sur 230323_trainmos entier
...
for i in range(imgnb):  # parcours des lignes (images)
    ...
    for j in range(len(mask_selected_feat)):  # parcours des colonnes (features)
        ...
        # a noter que la variable X_train vaut en fait X_test quand on fait tourner ce code pour recuperer Xtestfiltered
