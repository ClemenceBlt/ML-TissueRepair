...
# Fichier pour lancer entrainement sur des reseaux de neurones depuis images qu'on enregistre dans fichier h5 decoupees
# en folds.
...
parser.add_argument('-lep', '--lastepoch', default=0, type=int)
...
if suffix == '_':
    prefix_name = 'mice_%s_%s_%s_ep%s_%s_bs%s_lr%s_wd%s_validfld%s' \
                  % (echeance_name, neuralnet_used, dataset_name, start_epoch, last_epoch, batch_size, learning_rate,
                     w_decay, valid_fold_nb)
    prefix_name_toload = 'mice_%s_%s_%s_ep0_%s_bs%s_lr%s_wd%s_validfld%s' \
                         % (echeance_name, neuralnet_used, dataset_name, start_epoch - 1, batch_size, learning_rate,
                            w_decay, valid_fold_nb)
    # prefix_name_toload = 'mice_%s_%s_%s_ep100_%s_bs%s_lr%s_wd%s_validfld%s'\
    #                     % (echeance_name, neuralnet_used, dataset_name, start_epoch - 1, batch_size, learning_rate,
    #                        w_decay, valid_fold_nb)
else:
    prefix_name = 'mice_%s_%s_%s_ep%s_%s_bs%s_lr%s_wd%s_validfld%s_%s' \
                  % (echeance_name, neuralnet_used, dataset_name, start_epoch, last_epoch, batch_size, learning_rate,
                     w_decay, valid_fold_nb, suffix)
    prefix_name_toload = 'mice_%s_%s_%s_ep0_%s_bs%s_lr%s_wd%s_validfld%s_%s' \
                         % (echeance_name, neuralnet_used, dataset_name, start_epoch - 1, batch_size, learning_rate,
                            w_decay, valid_fold_nb, suffix)
    # prefix_name_toload = 'mice_%s_%s_%s_ep100_%s_bs%s_lr%s_wd%s_validfld%s_%s' \
    #                   % (echeance_name, neuralnet_used, dataset_name, start_epoch - 1, batch_size, learning_rate,
    #                     w_decay, valid_fold_nb, suffix)
...
f.write('mice_classif_otherNet_fldbyfld.py\n')
print('mice_classif_otherNet_fldbyfld.py')
...
txt_file = open('./text_files/folds%s_%s_%s_divmos.txt' % (str(kfolds), echeance_name, dataset_name))
...
mice_numbers = ["nalox_j03_01", 'nalox_j03_01_OGG', 'nalox_j03_02', 'nalox_j03_02_ODD', 'nalox_j03_03',
                'nalox_j03_03_OG', 'nalox_j03_04_ODD', 'ptlip_j03_01_OG', 'ptlip_j03_01_OD', 'nalox_j10_01',
                'nalox_j10_01_OG', 'nalox_j10_02', 'nalox_j10_02_OG', 'ptlip_j10_01_OG', 'ptlip_j10_01_OD',
                'ptlip_j10_02_OD']
# version avec 16 mice (pour la classif 16miceTV/16miceTe)
    ...
    for k in range(folds_number):  # parcours des folds successifs
        ...
        for lsmczi_imgname in all_folds[k]:  # parcours des images du fold k
            ...
            for tif_ch1img_name in tif_ch1_names:
                ...
                mice_ear = lsmczi_imgname[22:27]  # lettres de l'oreille en incluant l'underscore d'apres
...
def train_epoch(neuralnet, gpudevice, epoch, train_loader, losscriterion, name_optimizer):
    ...
    for data in train_loader:  # passage successif dans les batchs
        ...
        _, predicted = torch.max(outputs, dim=-1)  # le label predit est l'indice du plus grand output parmi les 22
...
def train(valid_fold, folds_number, pathh5, used_nn, transformvalid, transformtrain, bs, wdecay, start_ep,
          last_ep, losscriterion, prefixname, prefixname_toload, transformaleacolor=None):
    '''
    :param valid_fold : numero de fold pour ensemble de valid
    ...
    if start_ep == 0:  # entrainement depuis zero
        ...
        for bestmodel in list(os.listdir('./tar_files/%s/best_models' % prefixname_toload)):
            ...
            if bestmodel[4] == str(valid_fold):
                ...
                f.write("Meilleur modele prealablement enregistre : {md}, a l'epoque {ep} avec loss de validation de {ls} \n"
...
print("Used loss:", criterion)
f.write("Used loss: {val} \n".format(val=criterion))

start = time.time()

fold_results = train(valid_fold_nb, kfolds, path_h5, neuralnet_used, transform_valid, transform_train, batch_size,
                     w_decay, start_epoch, last_epoch, criterion, prefix_name, prefix_name_toload, transform_alea_color)
...
# Fonctions objectifs
def plot_loss(resultats, valid_fold, used_nn, start_ep, last_ep, losscriterion, lr, bs, wdecay, prefixname, datasetname,
              sfx):
    fig, ax = plt.subplots()
...
plot_loss(fold_results, valid_fold_nb, neuralnet_used, start_epoch, last_epoch, criterion, learning_rate, batch_size,
          w_decay, prefix_name, dataset_name, suffix)


# Trace de l'accuracy
def plot_accuracy(resultats, valid_fold, used_nn, start_ep, last_ep, losscriterion, lr, bs, wdecay, prefixname,
                  datasetname, sfx):
    fig, ax = plt.subplots()
    ax.set_xlabel('Number of epochs')
